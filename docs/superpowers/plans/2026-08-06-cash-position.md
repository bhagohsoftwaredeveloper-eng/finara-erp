...
// Account 1011 as it actually stands in the books.
const GL_1011 = [
  { date: '2026-08-03', in: 0,    out: 30 },
  { date: '2026-08-05', in: 7830, out: 7890 },
];

// What the Daily Remittance Report freezes onto each record: the day's outflow.
const DAILY_OUT = {
  '2026-08-03': 30,
  '2026-08-04': 0,
  '2026-08-05': 7890,
...
describe('the two cash reports agree', () => {
  const { rows, opening, closing, totalOut } = buildCashbook(0, GL_1011);
  const byDate = Object.fromEntries(rows.map((r) => [r.date, r]));

  test.each(Object.keys(DAILY_OUT))(
    'cash position outflow for %s equals the daily remittance figure',
    (date) => {
      expect(byDate[date]?.out ?? 0).toBe(DAILY_OUT[date]);
    },
  );

  test('the daily figures sum to the cash position total', () => {
    const summed = Object.values(DAILY_OUT).reduce((s, n) => s + n, 0);
    expect(round2(summed)).toBe(totalOut);
  });

  test('opening plus net movement equals closing', () => {
    const net = GL_1011.reduce((s, m) => s + m.in - m.out, 0);
    expect(closing).toBe(round2(opening + net));
  });

  test('a day the daily report calls zero produces no cashbook row', () => {
    expect(byDate['2026-08-04']).toBeUndefined();
    expect(DAILY_OUT['2026-08-04']).toBe(0);
  });
});
```
